...
const Profile = () => {
  ...
  //on reloading the saved profile information should be visible now so for this we'll use useEffect
  useEffect(() => {
  ...
  const handleImageChange = async (event) => {
    ...
    if (file) {
      ...
      formData.append("profile-image", file);   //same name as mentioned in input
  ...
  return (
    <div className="bg-[#1b1c24] h-[100vh] flex items-center justify-center flex-col gap-10">
      <div className="flex flex-col gap-10 w-[80vw] md:w-max">
        ...
        <div><IoArrowBack className="text-4xl lg:text-6xl text-white text-opacity-90 cursor-pointer" onClick={handleNavigate} /></div>
        <div className="grid grid-cols-2">
          <div className="h-full w-32 md:w-48 md:h-48 relative flex items-center justify-center" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
            ...
            {
              hovered && (
                <div className="absolute inset-0 flex items-center justify-center bg-black opacity-50 ring-fuchsia-50 rounded-full " onClick={image ? handleDeleteImage : handleFileInputClick} >
                  {
          ...
          <div className="flex min-w-64 flex-col gap-5 text-white items-center justify-center">
            ...
            <div className="w-full flex gap-5">
              {
                colors.map((color, index) => (
                  <div className={`${color} h-8 w-8 rounded-full cursor-pointer transition-all duration-300 ${selectedColor === index ? "outline outline-white/50  " : ""}`}
                    key={index}
