...
const MessageContainer = () => {
  ...
  const renderMessages = () => {
    ...
    return selectedChatMessages.map((message, index) => {
      ...
      return (
        <div key={index}>
          {showData && <div className="text-center text-gray-500 my-2">
          ...
          <div className={`${message.sender !== selectedChatData._id ? "bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50 " : "bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20 "}  border inline-block p-4 rounded my-1 max-w-[50%] break-words`}>
            {
              checkIfImage(message.fileUrl) ? (
                <div className="cursor-pointer"
                  ...
                  <img src={`${HOST}/${message.fileUrl}`} alt="imageurl" height={300} width={300} />
                </div>
              ) : (
          ...
          message.sender._id !== userInfo.id ?
            <div className="flex items-center justify-start gap-3 ">
              <Avatar className={'h-8 w-8 rounded-full overflow-hidden'}>
                ...
                <AvatarFallback className={`uppercase h-8 w-8 md:w-48 md:h-48 text-lg flex items-center justify-center rounded-full ${getColor(message.sender.color)}`} >
                  {message.sender.firstName ? message.sender.email.split("").shift() : message.sender.email.split("").shift()}
                </AvatarFallback>
              </Avatar>
      ...
      <div ref={scrollRef}>
        {
          showImage && <div className="fixed z-[1000] top-0 left-0 h-[100vh] w-[100vw] flex items-center justify-center backdrop-blur-lg flex-col">
            <div>
              <img src={`${HOST}/${imageURL}`} alt="imageurl"
